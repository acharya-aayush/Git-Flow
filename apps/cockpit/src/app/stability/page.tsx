              ...
              {rows.map((row) => (
                <tr key={row.repo} className="border-b border-border/40 last:border-0">
                  ...
                  <td className="px-4 py-2">
                    <span className={`inline-flex rounded-sm border px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${row.stabilityScore >= 70 ? 'border-emerald-500/35 bg-emerald-500/15 text-emerald-200' : row.stabilityScore >= 40 ? 'border-amber-500/35 bg-amber-500/15 text-amber-200' : 'border-rose-500/35 bg-rose-500/15 text-rose-200'}`}>
                      {row.stabilityScore}
                    </span>
                  </td>
